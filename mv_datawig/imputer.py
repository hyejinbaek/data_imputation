...
class Imputer:
    ...
    def __init__(self,
        ...
        print(" self.is_explainable :::::: ", self.is_explainable)
        print(" =-=== self.data_featurizers ===", type(self.data_featurizers))
        print(" =-=== self.data_encoders ===", type(self.data_encoders))
        print(" =-=== self.label_encoders ===", type(self.label_encoders))
        ...
        for encoder in self.data_encoders: # len(data_encoders) = 13
            ...
            print("== data_encoder encoder_type == ", encoder_type)
        ...
        print(" !!!! encoder !!!! ", encoder)
        print(" !!!! self.data_encoders !!!! ", self.data_encoders)
...
def main():
    ...
    for col in train_col:
        data_encoders = train_col
        data_featurizers = train_col
        label_encoders = col
        output_path = 'imputer_model'

    # 클래스 인스턴스 생성
    instance = Imputer(data_encoders, data_featurizers, label_encoders, output_path)

    # 클래스의 다른 메서드 호출이나 작업 수행
    # 예시: 클래스의 메서드 호출
