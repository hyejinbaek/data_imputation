...
# CSV 파일 경로 설정
result_csv_path = '/userHome/userhome2/hyejin/test/res/chain2/23_ai4i_datawig_method_res.csv'
...
def main():
    ...
    prepro_data = '/userHome/userhome2/hyejin/paper_implementation/00_dataset/preprocessing/23_ai4i.csv'
    prepro_data = pd.read_csv(prepro_data)

    data_pth = '/userHome/userhome2/hyejin/paper_implementation/00_dataset/missing/23_ai4i.csv'
    df_data = pd.read_csv(data_pth)
    train_col = ['Air temperature', 'Process temperature', 'Rotational speed', 'Torque', 'Tool wear']
    ...
    for iteration in range(num_iterations):
        ...
        # 데이터 결측치 채우기
        for i, col in enumerate(train_col):
            input_columns = train_col[:i+1]  # 현재 컬럼까지의 모든 컬럼
            print("==== input_columns ===", input_columns)
            imputer = SimpleImputer(
                input_columns=input_columns,
                output_column=col,
                output_path=f'./imputer_model/23_ai4i/imputer_model_{col}',
