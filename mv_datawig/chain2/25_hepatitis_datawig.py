...
# CSV 파일 경로 설정
result_csv_path = '/userHome/userhome2/hyejin/test/res/chain2/25_hepatitis_datawig_method_res.csv'
...
def main():
    ...
    prepro_data = '/userHome/userhome2/hyejin/paper_implementation/00_dataset/preprocessing/25_hepatitis.csv'
    prepro_data = pd.read_csv(prepro_data)

    data_pth = '/userHome/userhome2/hyejin/paper_implementation/00_dataset/missing/25_hepatitis.csv'
    df_data = pd.read_csv(data_pth)
    train_col = ['age', 'sex', 'steroid', 'antivirals', 'fatigue', 'malaise', 'anorexia', 'liver_big', 'liver_firm', 'spleen', 'souders', 'ascites', 'varices', 
                   'bilirubin', 'alk', 'sgot', 'albumin', 'protime', 'histology']
    
    # train_col 리스트를 랜덤하게 섞음
    random.shuffle(train_col)
    ...
    for i in range(num_iterations):
        ...
        for col in train_col:
            ...
            imputer = SimpleImputer(
                ...
                output_column=col,
                output_path=f'./imputer_model/25_hepatitis/imputer_model_{col}',
            ...
            # 컬럼별 RMSE 값을 저장
            result = {
                'Dataset': '25_hepatitis',
                'method': 'chain02_datawig',
                'Iteration': i + 1,
                'Random_train_col' : train_col,
                'Column': col,
